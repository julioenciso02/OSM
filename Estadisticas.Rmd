...
p <- read_excel_url("https://github.com/julioenciso02/OSM/raw/main/BDF.xlsx")
...
res_of <- data.frame(Goles = df_limpio$Goles, Asistencias = df_limpio$Asistencias,
                     Pases = paste0("(",df_limpio$Pas_comp,"/",df_limpio$Pas_int,")","= ",round(df_limpio$Prec_pas*100,0),"%"),"TirosAPuerta" = df_limpio$Tir_p, "TirosFuera" = df_limpio$Tir_f, Centros = paste0("(",df_limpio$Cent_c,"/",df_limpio$Cent_in,")","= ",round(df_limpio$Prec_cent*100,0),"%"),Regates =paste0("(",df_limpio$Reg_comp,"/",df_limpio$Reg_int,")","= ",round(df_limpio$Prec_reg*100,0),"%"),"FuerasDeJuego" = df_limpio$Fdj,"GranOcasCreada" = df_limpio$Gran_oc_cr, "GranOcasFallada" = df_limpio$Gran_oc_fall)
tt <- flextable(res_of)
tt <- theme_flextable(tt)
```

```{r demas}
res_otr <- data.frame("FaltasRecibidas" = df_limpio$Falt_rec, "FalrasCometidas" = df_limpio$Falt_com, Despejes = df_limpio$Despejes, Recuperaciones = df_limpio$Recuperaciones,"PosesiónPerdida" = df_limpio$Pos_per, "GolesAFavor" = df_limpio$GF, "GolesEnContra" = df_limpio$GC, "DiferenciaGoles" = df_limpio$DG,"Amarilla" = df_limpio$Amarillas, Roja = df_limpio$Roja)
...
`r paste(df_limpio$Gran_oc_cr, "Asistencias de gol", ifelse(pf_granocc > 0, paste("\\textcolor{green}{\\textbf{", pf_granocc, "}}"), ifelse(pf_granocc < 0, paste("\\textcolor{red}{\\textbf{", pf_granocc, "}}"), pf_granocc)),"\n")`  
